...
class OpenRouterRequest(Base):
    ...
    media_type_processed = Column(
        String, nullable=True
    )  # e.g., 'image', 'video', 'image_link', 'video_link'
    ...
    )  # Router's own processing time
    openrouter_latency_ms = Column(
        Integer, nullable=True
    )  # Latency of the call to OpenRouter
    status_code_returned_to_client = Column(Integer, nullable=False)
    error_source = Column(
        String, nullable=True
    )  # e.g., 'router', 'openrouter', 'media_processing'
    error_message = Column(Text, nullable=True)
    is_streaming = Column(Boolean, default=False, nullable=False)  # Add this line
